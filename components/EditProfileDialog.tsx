...
export default function EditProfileDialog({ initialData }: EditProfileDialogProps) {
  ...
  const handleComplete = () => {
    ...
    setOpen(false);
    // You can also trigger a refresh of parent data here if needed
  };
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button
          variant="outline"
          className="mt-4 sm:mt-0 text-sm h-8 rounded-xl bg-white/50 border-white/50 hover:bg-white/80 transition-colors shadow-sm text-gray-700 font-semibold"
        >
          Edit Profile
        </Button>
      </DialogTrigger>
      <DialogContent className="sm:max-w-2xl bg-transparent border-none shadow-none p-0 overflow-visible [&>button]:hidden">
        ...
        <div className="relative">
          ...
          {/* Pass onComplete callback to close dialog after save */}
          <OnboardingForm

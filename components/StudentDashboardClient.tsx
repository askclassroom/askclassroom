...
const StudentDashboardClient = ({ data }: StudentDashboardClientProps) => {
    ...
    // Enhanced data for charts
    const subjectPerformanceData = (allSubjectsPerformance || subjectsStudied.map((subject, index) => ({
        subject,
        sessions: Math.floor(Math.random() * 20) + 5,
        timeSpent: Math.floor(Math.random() * 300) + 100,
        progress: Math.floor(Math.random() * 40) + 60,
    ...
    return (
        ...
        >
            ...
            {/* Subject Performance Section - New addition matching your image */}
            {allSubjectsPerformance && allSubjectsPerformance.length > 0 && (
                <motion.div
                    initial={{ opacity: 0, y: 20 }}
                    animate={{ opacity: 1, y: 0 }}
                ...
                >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                        {allSubjectsPerformance
                            ...
                            .map((subject, index) => {
                                const label = getPerformanceLabel(subject.scorePercentage);
                                const colorClass = getPerformanceColor(subject.scorePercentage);
                                const bgColorClass = getPerformanceBgColor(subject.scorePercentage);
                                ...
                                return (
                                    <motion.div
                                        ...
                                        onClick={() => {
                                            const type = subject.scorePercentage >= 80 ? 'Strong' :
                                                subject.scorePercentage <= 40 ? 'Weak' : 'Strong';
                                            setSelectedSubject({
                                                data: subject,
                                                type: type as 'Strong' | 'Weak'
                                            });
                                        }}
                                        className={cn(
                                            "p-4 rounded-xl border-2 transition-all cursor-pointer hover:shadow-md",
                                            index === 0 ? "border-green-200 bg-green-50/30" :
                                                index === allSubjectsPerformance.length - 1 ? "border-red-200 bg-red-50/30" :
                                    ...
                                    >
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            <div className="h-2 bg-gray-200 rounded-full overflow-hidden">
                                                <motion.div
                                                    initial={{ width: 0 }}
                                                    animate={{ width: `${subject.scorePercentage}%` }}
                                                    transition={{ duration: 1, delay: 0.5 + index * 0.1 }}
                                                    className={cn(
                                                        "h-full rounded-full",
                                                        label === 'strong' ? "bg-green-500" :
                                                            label === 'good' ? "bg-blue-500" :
                                                                label === 'avg' ? "bg-yellow-500" :
                                                                    "bg-red-500"
                                                    )}
                                                />
                                            </div>
                                            ...
                                            <div className="flex justify-between text-xs text-gray-500 mt-2">
                                                <span>{subject.sessionsCount} sessions</span>
                                                <span>{Math.round(subject.totalTimeMinutes / subject.sessionsCount)} min/session</span>
                                            </div>
                                        </div>
                                        ...
                                        {index === allSubjectsPerformance.length - 1 && allSubjectsPerformance.length > 1 && (
                    ...
                    <div className="mt-6 pt-6 border-t border-gray-100 grid grid-cols-2 md:grid-cols-4 gap-4">
                        <div className="text-center">
                            <div className="text-2xl font-bold text-gray-900">{allSubjectsPerformance.length}</div>
                            <div className="text-xs text-gray-500">Total Subjects</div>
                        </div>
                        <div className="text-center">
                            <div className="text-2xl font-bold text-green-600">
                                {allSubjectsPerformance.filter((s: any) => s.scorePercentage >= 80).length}
                            </div>
                            <div className="text-xs text-gray-500">Strong (80%+)</div>
                        </div>
                        <div className="text-center">
                            <div className="text-2xl font-bold text-yellow-600">
                                {allSubjectsPerformance.filter((s: any) => s.scorePercentage >= 60 && s.scorePercentage < 80).length}
                            </div>
                            <div className="text-xs text-gray-500">Good (60-79%)</div>
                        </div>
                        <div className="text-center">
                            <div className="text-2xl font-bold text-red-600">
                                {allSubjectsPerformance.filter((s: any) => s.scorePercentage < 60).length}
                            </div>
                            <div className="text-xs text-gray-500">Needs Work (&lt;60%)</div>
                        </div>
                    </div>
                </motion.div>
            )}
